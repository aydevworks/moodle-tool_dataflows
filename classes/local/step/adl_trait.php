...
trait adl_trait {
     ...
     */
    private function generate_auth_header(string $method, string $contentlength, string $contenttype, string $date, string $path, string $querystring = '', array $additionalheaders = []): string {
        if ($contentlength === '0') {
            $contentlength = '';
        }
    ...
    public function copy_blob(string $sourcepath, string $destinationpath): bool {
        ...
        try {
            ...
            throw new \Exception("Failed to copy blob: " . $e->getMessage());
        }
    }
    ...
    public function upload_blob(string $path, string $content, string $contenttype = 'application/octet-stream'): bool {
        ...
        try {
            ...
            throw new \Exception("Failed to upload blob: " . $e->getMessage());
        }
    }
    ...
    public function download_blob(string $path): ?string {
        ...
        try {
            ...
            throw new \Exception("Failed to download blob: " . $e->getMessage());
        }
    }
    ...
    public function execute($input = null): mixed {
        ...
        $source_is_adl = $this->is_adl_path($config->source);
        $target_is_adl = $this->is_adl_path($config->target);

        // Resolve the actual paths
        $source_path = $this->resolve_path($config->source, $source_is_adl);
        $target_path = $this->resolve_path($config->target, $target_is_adl);

        // Do not execute ADL operations during a dry run.
        if ($this->enginestep->engine->isdryrun) {
            $this->enginestep->log("Skipping copy to '{$target_path}' as this is a dry run.");
            return $input;
        }

        // Determine the operation type based on source and target
        if (!$source_is_adl && $target_is_adl) {
            // Local to ADL
            $content = file_get_contents($source_path);
            $mime_type = mime_content_type($source_path) ?: 'application/octet-stream';
            $this->upload_blob($target_path, $content, $mime_type);
        } else if ($source_is_adl && !$target_is_adl) {
            // ADL to Local
            $content = $this->download_blob($source_path);
            file_put_contents($target_path, $content);
        } else if ($source_is_adl && $target_is_adl) {
            // ADL to ADL - Use server-side copy
            $this->copy_blob($source_path, $target_path);
        }
